...
OWN.ru = {
  ...
  note_csp: 'Политика, разрешающая встроенные скрипты, ничем не мешает скрипту, внедрённому со стороны. Nonce и хеши существуют ровно для того, чтобы этого не требовалось, — а политика, где рядом стоят nonce и unsafe-inline, означает разные вещи в двух поколениях браузеров.',
  ...
  cap_csp_allows_inline_script: 'политика разрешает встроенные скрипты',
  ...
  fd_certificate_not_trusted_on_alias: 'Канонический адрес в порядке, а другое имя — обычно www — сертификатом не покрыто. curl и редирект при этом работают, поэтому такое проходит тестирование незамеченным, и жалуются только браузеры.',
  ...
  fd_csp_missing: 'Ничто не ограничивает, откуда можно грузить скрипты и можно ли выполнять встроенные. CSP — единственный заголовок, который ограничивает ущерб от внедрения, а не пытается его предотвратить.',
  ...
  flag_csp_unsafe_inline: 'Политика разрешает встроенные скрипты',
  ...
  fd_csp_bypassable_host: 'Часть крупных CDN раздаёт произвольные библиотеки или отвечает на JSONP-запросы, подставляя в ответ имя функции прямо из запроса. Разрешить такой хост — почти то же, что разрешить любой скрипт. Закрывается через strict-dynamic или указанием конкретных путей.',
  ...
  flag_csp_unsafe_inline_style: 'Политика разрешает встроенные стили',
  ...
  fd_hsts_sent_over_http: 'RFC 6797 §8.1 предписывает браузерам игнорировать заголовок на незашифрованном ответе. Безвредно и обычно означает, что его добавили не в тот блок настроек.',
  ...
  fd_no_nosniff: 'Без nosniff браузер может решить, что ответ — это скрипт, хотя он отдавался как что-то другое; так адрес для загрузки файлов превращается в хостинг скриптов.',
  ...
  fd_no_coop: 'COOP разрывает связь между окном страницы и тем, что она открывает, что закрывает целое семейство межоконных атак и открывает путь к изоляции между источниками.',
  ...
  fd_cors_wildcard_with_credentials: 'Браузеры отвергают это сочетание целиком, поэтому такой ответ не только слишком открыт, но ещё и сломан. Источники нужно перечислить явно.',
