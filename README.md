...
```json
"@sharapov/service-kit": "git+https://github.com/sharapov-outsource/service-kit.git#v1.5.1"
```
...
securityheaders.com — хороший инструмент, и рекламы там нет, так что лёгкой добычи
здесь нет. Единственный способ быть полезнее — уйти глубже в тот
...
- `data:` в `script-src` — это скрипт, который злоумышленник соберёт, ничего
  нигде не размещая;
...
ровно того визита, на который и нацелится злоумышленник в сети.

Сертификат, который не проходит проверку на псевдониме, сообщается отдельно от
того, который не проходит на каноническом имени. Непокрытое имя `www` — частый
...
так и не подав ни одной заявки, и все причастные будут считать, что дело
...
молча не работает. Снаружи видны только куки, установленные на конечной
странице, — отчёт об этом и говорит.

**Протоколы.** HTTP/2 проверяется открытием рукопожатия TLS и чтением того, что
выбрал сервер. HTTP/3 — так, как он объявлен в `Alt-Svc`, и помечен именно как
объявление: этот сканер не говорит на QUIC, и ответ «нет» был бы выдуманным.
Сжатие измеряется запросом всего подряд и разбором того, что вернулось.

### Оценка

Транспорт 35%, CSP 40%, остальные заголовки 25%. У CSP наибольший вес по
названной выше причине: политика, которая есть, но обходится, получает столько же, сколько
...
npm run scan -- example.com           # проверка из консоли, без веб-сервера
...
URL выписан полностью, потому что сокращение `github:` разворачивается в
