 ...
 * start a check aims at a name that cannot resolve, so it fails fast whatever
 * the network is doing; it is asserted only on the shape of the stream. The
 * reasoning is covered by unit tests that work on captured header sets.
 */
...
async function run() {
  ...
  check('healthz: names itself', health.service === 'myheaders');
  ...
  check('html: translated title', /<title>Проверка заголовков/.test(russian),
  ...
  check('usage: lists the stages', Array.isArray(usageBody.stages) && usageBody.stages.includes('csp'));
  check('usage: lists the family', usageBody.family?.some(entry => entry.slug === 'mymx'));
  ...
  check('stream: a site that does not answer does not crash the checker',
    !streamBody.includes('"error":"scan-failed"'),
    streamBody.slice(-300));

  /* The preload list is data in the repository, and /healthz says whether this
     deployment actually has it — reporting "not preloaded" without it would be
     a false negative for every preloaded site. */
  const preload = health.hstsPreloadList;
  check('healthz: reports whether the preload list is bundled',
    typeof preload?.bundled === 'boolean', JSON.stringify(preload));
  if (preload?.bundled) {
    check('healthz: the preload list has entries', preload.entries > 10000, String(preload.entries));
  }
